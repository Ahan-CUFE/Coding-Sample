...
trading_frictions = {'beta', 'pps', 'baspread', 'SMB', 'IPO', 'turn', 'mve_ia', 'dolvol', 'std_dolvol', 'std_turn', 'ill', 'LIQ_PS', 'idiovol', 'retvol', 'zerotrade', 'sin', 'stdcf', 'stdacc', 'maxret', 'BAB', 'Intermediary'};
trading_frictions_ind = find(ismember(factorname,trading_frictions));
...
result=[];
for i = 1:length(trading_frictions)
    disp(i)
    gt = factors(:,trading_frictions_ind(i))'; % test factor
    j=trading_frictions_ind(i);
...
writetable(result,'trading_frictions.csv')