...
intangibles = {
    'cashdebt', 'kz', 'currat', 'ob_a', 'pchcurrat', 'roavol', 'pchquick', 'pricedelay', 'pchsaleinv', 'age', 'quick', 'herf', 'salecash', 'ww', 'saleinv', 'tang', 'salerec', 'moms12m', 'pchgm_pchsale', 'absacc', 'pchsale_pchinvt', 'invest', 'pchsale_pchrect', 'realestate_hxz', 'pchsale_pchxsga', 'pctacc', 'etr', 'ol', 'lfe', 'cash', 'pchcapx_ia', 'orgcap', 'adm', 'gad', 'rdm', 'ala', 'rds', 'convind'
};

intangibles_ind = find(ismember(factorname,intangibles));
...
result=[];
for i = 1:length(intangibles)
    disp(i)
    gt = factors(:,intangibles_ind(i))'; % test factor
    j=intangibles_ind(i);
...
writetable(result,'intangibles.csv')