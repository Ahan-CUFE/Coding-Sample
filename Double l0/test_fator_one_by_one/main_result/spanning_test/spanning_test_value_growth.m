...
value_growth = {'MktRf', 'ep', 'dy', 'LTR', 'lev', 'depr', 'pchdepr', 'HML', 'cp', 'divi', 'divo', 'sp', 'bm_ia', 'cfp_ia', 'cfp', 'tb', 'op', 'nop', 'ndp', 'ebp', 'cashpr', 'em', 'HML_Devil', 'QMJ'};
value_growth_ind = find(ismember(factorname,value_growth));
...
result=[];
for i = 1:length(value_growth)
    disp(i)
    gt = factors(:,value_growth_ind(i))'; % test factor
    j=value_growth_ind(i);
...
writetable(result,'spanning_test_value_growth.csv')