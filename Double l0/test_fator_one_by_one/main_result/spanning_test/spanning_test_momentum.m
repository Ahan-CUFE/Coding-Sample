...
momentum = {'sue', 'STR', 'mom6m', 'mom36m', 'UMD', 'nincr', 'indmom', 'chmom', 'rs', 'aeavol', 'ear', 'rsup'};
momentum_ind = find(ismember(factorname,momentum));
...
for i = 1:length(momentum)
    disp(i)
    gt = factors(:,momentum_ind(i))'; % test factor
    j=momentum_ind(i);
...
writetable(result,'spanning_test_mom.csv')