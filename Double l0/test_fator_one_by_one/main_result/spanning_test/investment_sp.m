...
investment = {'acc', 'chinv', 'chtx', 'grltnoa', 'grltnoa_hxz', 'rd', 'cinvest', 'cinvest_a', 'dsti', 'dfnl', 'egr_hxz', 'grcapx', 'pchcapx3', 'cei', 'nef', 'ndf', 'noa', 'dnoa', 'egr', 'lgr', 'dcoa', 'dcol', 'dwc', 'dnca', 'dncl', 'dnco', 'dfin', 'ta','nxf', 'roic', 'chcsho', 'dpia', 'pchcapx', 'cdi', 'ivg', 'poa', 'hire', 'CMA', 'HXZ_IA'};
investment_ind = find(ismember(factorname,investment));
...
result=[];
for i = 1:length(investment)
    disp(i)
    gt = factors(:,investment_ind(i))'; % test factor
    j=investment_ind(i);
...
writetable(result,'investment.csv')