...
reccoff_DBL_hbic = zeros(Sim_time,p1+p2);
reccoff_DBL_hbic1 = zeros(Sim_time,p1+p2);
reccoff_DBL_hbic2 = zeros(Sim_time,p1+p2);
reccoff_DBL_hbic3 = zeros(Sim_time,p1+p2);
...
%% double lasso  and single lasso 
    ...
    % t_bias_DBL = [t_bias_DBL (lambda_ds-lambdag)./model_ds.se_ds ];
    % 
    ...
    % t_SL      = [t_SL ;tstat_ss];
    % t_bias_sl  = [t_bias_sl (lambda_ss-lambdag)./model_ds.se_ss];
    % 
...
% 
%      t_bias_Dl0_hbic = [t_bias_Dl0_hbic (model_DL0_hbic.lambdag_ds-lambdag)./model_DL0_hbic.se_ds ];
%      t_bias_sl0_hbic = [t_bias_sl0_hbic (model_DL0_hbic.lambdag_ss-lambdag)./model_DL0_hbic.se_ss];
% 
% 
...
% 
%      t_bias_Dl0 = [t_bias_Dl0 (model_DL0.lambdag_ds-lambdag)./model_DL0.se_ds ];
%      t_bias_sl0 = [t_bias_sl0 (model_DL0.lambdag_ss-lambdag)./model_DL0.se_ss];
% 
% 
...
%%  post-lasso 
    ...
    Kfld = 10;
    sim_times = 1;
    model_DBS_hbic = Postlaso_hbic_forsimu(Ri,gt,ht,opts3,Kfld,200,sim_times);
    lam_DBL_hbic = [lam_DBL_hbic model_DBS_hbic.lambdag_ds];
    tstat_DBL_hbic = model_DBS_hbic.lambdag_ds./model_DBS_hbic.se_ds;
    t_DBL_hbic = [t_DBL_hbic tstat_DBL_hbic];
    t_bias_DBL_hbic = [t_bias_DBL_hbic (model_DBS_hbic.lambdag_ds - lambdag)./model_DBS_hbic.se_ds];
    reccoff_DBL_hbic(i,model_DBS_hbic.select) = 1;
    reccoff_DBL_hbic1(i,model_DBS_hbic.sel1) =1;
    reccoff_DBL_hbic2(i,model_DBS_hbic.sel2) =1;
    
    reccoff_DBL_hbic3(i,model_DBS_hbic.sel3) =1;
end
