...
if opts.isCV == 1   
    for j = 1:Jrep
        ...
        for k = 1:Kfld
            ...
            model1 = AESDAR((Ch_train-mean(Ch_train))*(diag(penalty)),ER_train-mean(ER_train),opts1);
            % cvm1(i,k,Jrep) = model1.Bic;
            ...
            model2 = AESDAR((Ch_train-mean(Ch_train))*(diag(penalty)),Cg_train-mean(Cg_train),opts2);
            % cvm2(jj,k,Jrep) = model2.Bic;
            ...
            % opts3.mu1  = opts.mu1;
            % N3 = size(ht_train,2);
            % opts3.mu1 = (N3/opts.tau0)/N3;
            
            N3 = size(ht_train,2);
            % opts3.mu1 = sqrt(N3)/N3;
            ...
            % cvm3(ii,k,Jrep) = model3.Bic;
            coff3 = model3.solutionpath;

    
            ER_pred = [(Ch_test-mean(Ch_test))*(diag(penalty))]*coff1;
            Cg_pred = [(Ch_test-mean(Ch_test))*(diag(penalty))]*coff2;
            ...
            cvm1(1:LL1,k,j) = mean((repmat(ER_test-mean(ER_test),1,LL1) - ER_pred).^2,1)';
            cvm2(1:LL2,k,j) = mean((repmat(Cg_test-mean(Cg_test),1,LL2) - Cg_pred).^2,1)';
...
%   normalize the X 
    ...
    opts1.outputmode = opts.outputmode;
    opts1.J = opts.J;
    opts1.tau =opts.taufix1;

    model1 = AESDAR((Ch0-mean(Ch0))*(diag(penalty)),ER0-mean(ER0),opts1);
    
    opts2.tau0 = 1;
    opts2.mu1  =  opts.mu1;
    opts2.outputmode = opts.outputmode;
    opts2.J = opts.J;
    opts2.tau =opts.taufix2;
    model2 = AESDAR((Ch0-mean(Ch0))*(diag(penalty)),Cg0-mean(Cg0),opts2);
    
    opts3.tau0 = 1;
    opts3.mu1  = opts.mu1;
    opts3.outputmode = opts.outputmode;
    opts3.J = opts.J;
    opts3.tau =opts.taufix3;
    model3 = AESDAR((ht(:,nomissing)-mean(ht(:,nomissing)))',(gt(:,nomissing)-mean(gt(:,nomissing)))',opts3);
end
